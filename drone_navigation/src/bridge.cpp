...
Realsense_Bridge::Realsense_Bridge(const ros::NodeHandle& nh)
    : nh_(nh) {

	odom_sub_ = nh_.subscribe<const nav_msgs::Odometry&>(
		"/camera/odom/sample_throttled", 10, &Realsense_Bridge::odomCallback, this);
	...
	worker_ = std::thread(&Realsense_Bridge::publishSystemStatus, this);
};

Realsense_Bridge::~Realsense_Bridge() { }


void Realsense_Bridge::odomCallback(const nav_msgs::Odometry& msg) {
...
void Realsense_Bridge::publishSystemStatus(){
  ...
  while(ros::ok()){
	...
	// only send heartbeat if we receive pose estimates at all
